...
for a in articles:
    words.extend( tokenize( a['text'] ) )

withoutStopwords(words)


reducedArray = [ { "word":wrd,"frequency":wordDictionary[wrd],"probability" : 0 } for(wrd) in wordDictionary ]
...
allBigrams = [
        {
            "first": fw,
            "second": sw,
            "frequency": bigramDictionary[fw][sw],
            "probability": 0
        }
        for fw in bigramDictionary
        for sw in bigramDictionary[fw]
]

allBigrams = sorted(allBigrams , key=lambda wd:wd["frequency"] , reverse=True)

for i in reducedArray:
    WORDS.insert_one(i)


for i in allBigrams:
    BIGRAMS.insert_one(i)
